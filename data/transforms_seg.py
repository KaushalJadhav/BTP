...
class RandomBlur():
    ...
    bg_mask : (H, W) numpy int32
    '''
    def __init__(self,max_magnitude=10):
    ...
    def __call__(self, img, masks):
        blur_model = random.choice(self.blur_bank)
        img = blur_model.augment_image(img)
        return img, masks
...
class GaussianNoise():
    ...
    '''
    def __init__(self,max_magnitude=10):
        range = np.linspace(0.03*255, 0.08*255, max_magnitude)
        self.noise_bank = []
        for val in range:
            self.noise_bank.append(iaa.AdditiveGaussianNoise(scale=val))
